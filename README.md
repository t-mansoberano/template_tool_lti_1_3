# Plantilla de solución en .NET Core + Angular App

## Índice
1. [Introducción](docs/introduccion.md)
2. [Estructura de la Arquitectura](docs/estructura_arquitectura.md)
    - [Capa Application](docs/capa_application.md)
    - [Capa Infrastructure](docs/capa_infrastructure.md)
    - [Capa Server](docs/capa_server.md)
    - [Capa Client](docs/capa_client.md)
3. [Ventajas de la Arquitectura](docs/ventajas.md)
4. [Justificación Técnica](docs/justificacion_tecnica.md)
5. [Paquetes Externos Utilizados](docs/paquetes_externos.md)
6. [Pruebas Unitarias](docs/pruebas_unitarias.md)
    - [Pruebas en .NET Core](docs/pruebas_net_core.md)
    - [Pruebas en Angular](docs/pruebas_angular.md)
7. [Conclusión](docs/conclusion.md)
8. Anexos
   - [Diagrama de Dominio](docs/anexos_diagrama_dominio.md)
   - [Diagrama de Arquitectura](docs/anexos_diagrama_arquitectura.md)
---

Consulta cada sección para obtener información detallada sobre la arquitectura y el diseño del sistema.
